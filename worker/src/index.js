/**
 * Ketogenesis "Ask" endpoint — a Cloudflare Worker.
 *
 * The website is static and does all browsing and filtering in the
 * browser. This Worker exists for one reason: it holds the Cerebras API
 * key, which must never be shipped to a browser.
 *
 * Flow:  POST /  { q: "kuch meetha hai?" }
 *        -> retrieve the 5 most relevant items from menu.json
...
/* ---------- origin control ----------
   Access-Control-Allow-Origin is only a hint to browsers. curl, scripts
   and bots ignore CORS completely, so the same rule is enforced here on
   the server as well. With ALLOWED_ORIGIN set, a request whose Origin
   header does not match is refused outright.                          */
function cors(env) {
  return {
...
function originAllowed(request, env) {
  ...
  if (!allowed || allowed === "*") return true;   // open while developing
  ...
  async fetch(request, env) {
    if (request.method === "OPTIONS") {
      return new Response(null, {
        status: originAllowed(request, env) ? 204 : 403,
        headers: cors(env),
      });
    }
    ...
    if (!q) return json({ error: "Empty question" }, 400, env);
    ...
    if (!env.CEREBRAS_API_KEY) {
      ...
      return json({ answer: null, items }, 200, env);
    }
    ...
    try {
      ...
      return json({ answer: answer || null, items }, 200, env);
    } catch (err) {
